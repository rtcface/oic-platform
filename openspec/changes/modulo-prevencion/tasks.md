...
- [ ] 2.1 Create `src/app/prevencion/public/prevencion-public.component.ts` (with HTML/SCSS). Map in routing.
- [ ] 2.2 Add `p-tabView` in HTML with tabs for "Evidencia de Actividades" and "Quejas por Violencia Institucional".
- [ ] 2.3 Integrate `app-finder-oic` into `prevencion-public.component`. Use unidirectional binding to avoid change-detection loops.
- [ ] 2.4 Add `p-table` in the first tab to display activities data.
- [ ] 2.5 Add `p-chart` in the second tab to display complaints data.

## Phase 3: Core Admin Implementation

- [ ] 3.1 Create `src/app/prevencion/admin/prevencion-admin.component.ts` (with HTML/SCSS).
- [ ] 3.2 Map admin route in `prevencion-routing.module.ts` and protect it with standard auth guards.
- [ ] 3.3 Add form to capture activity details in the admin component.
- [ ] 3.4 Add form to capture complaint totals per municipality in the admin component.

## Phase 4: Testing & Verification

- [ ] 4.1 Write component tests for `PrevencionPublicComponent` verifying tabs and finder integration.
- [ ] 4.2 Write component tests for `PrevencionAdminComponent` verifying auth guards and form interactions.
